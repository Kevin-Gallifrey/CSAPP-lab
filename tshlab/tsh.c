...
void eval(char *cmdline) 
{
    ...
    int bg;
    pid_t pid;
    sigset_t mask, mask_all, old_set;
    ...
    if (builtin_cmd(argv) == 0)
    {
        ...
        sigfillset(&mask_all);
        sigprocmask(SIG_BLOCK, &mask, &old_set);  /* Block SIGCHLD */
        pid = fork();
        if (pid == 0)  /* Child */
        {
            sigprocmask(SIG_SETMASK, &old_set, NULL);  /* Unblock SIGCHLD */
...
void sigchld_handler(int sig) 
{
    ...
    if (pid < 0 && errno != ECHILD)  /* waitpid error */
